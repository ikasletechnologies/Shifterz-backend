...
export class OutpassService {
  ...
  async getAllOutpasses(userRole?: string, franchiseId?: string) {
    ...
    try {
      ...
      // 1. Process Invoices (Paid / Completed)
      for (const inv of allInvoices) {
        const invPayments = payments.filter(
          (p) => p.invoiceId === inv.id || (inv.jobId && p.jobId === inv.jobId)
        );
        const totalPaid = invPayments.reduce((sum, p) => sum + Number(p.amount || 0), 0);
        const invTotal = (inv.amount || 0) + (inv.gst || 0) - (inv.discount || 0);
        ...
        const isPaid =
          ...
          (invTotal > 0 && totalPaid >= invTotal - 1) ||
          (invPayments.length > 0 && totalPaid > 0 && totalPaid >= invTotal);

        if (isPaid) {
          const vehStr = inv.vehicle && inv.vehicle !== "-" ? inv.vehicle : "N/A";
          const normVeh = vehStr.replace(/[^A-Z0-9]/g, "").toUpperCase();
          const existingOutpasses = await db.outPass.findMany({
            where: { isDeleted: false },
          });

          const hasExisting = existingOutpasses.some(op => {
            if (op.invoiceId === inv.id) return true;
            if (inv.jobId && op.jobCardId === inv.jobId) return true;
          ...
          if (!hasExisting) {
            ...
            await db.outPass.create({
              data: {
                ...
                vehicle: vehStr,
                model: "",
                customer: inv.client || "Walk-in Customer",
                phone: inv.phone || "",
                service: inv.service || "General Service",
                outTime: new Date(),
                securityName: "",
                technicianName: "",
                remarks: "Auto-generated from completed payment",
                issued: false,
                status: "Pending",
                carInId: "",
                jobCardId: inv.jobId || null,
                invoiceId: inv.id,
                paymentStatus: "Paid",
                createdBy: "System",
                franchiseId: inv.franchiseId || null,
              },
            });
          }
        }
      ...
      // 2. Process Jobs with Paid / Payment Completed status
      for (const job of allJobs) {
        const jobStatusUpper = (job.status || "").toUpperCase();
        const isJobPaid =
          jobStatusUpper.includes("PAYMENT COMPLETED") ||
          jobStatusUpper.includes("PAID") ||
          jobStatusUpper.includes("COMPLETED");

        if (isJobPaid) {
          const vehStr = job.vehicle && job.vehicle !== "-" ? job.vehicle : "N/A";
          const normVeh = vehStr.replace(/[^A-Z0-9]/g, "").toUpperCase();
          const existingOutpasses = await db.outPass.findMany({
            where: { isDeleted: false },
          });
          ...
          const hasExisting = existingOutpasses.some(op => {
            if (op.jobCardId === job.id) return true;
            if (normVeh && normVeh !== "NA" && normVeh !== "N/A") {
              const opNormVeh = (op.vehicle || "").replace(/[^A-Z0-9]/g, "").toUpperCase();
              if (opNormVeh === normVeh) return true;
            }
          ...
          if (!hasExisting) {
            ...
            await db.outPass.create({
              data: {
                ...
                vehicle: vehStr,
                model: "",
                customer: job.customer || "Walk-in Customer",
                phone: "",
                service: job.service || "General Service",
                outTime: new Date(),
                securityName: "",
                technicianName: "",
                remarks: "Auto-generated from completed job payment",
                issued: false,
                status: "Pending",
                carInId: "",
                jobCardId: job.id,
      ...
      console.error("Auto outpass sync error:", err);
    }
